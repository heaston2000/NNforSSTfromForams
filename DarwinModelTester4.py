# This document is meant to iterate (10 trials each) over several architectures and record which
# ones perform best, eventually ending up with a working model with mimimum RMSE
# MORE COMPREHENSIVE MODEL TESTER TO FEED TO DARWIN
...
class SSTDataset(Dataset):
    ...
    def __init__(self):
        # Data loading
        dataAll = pd.read_csv('~/ForDarwin4/Training_Input_for_Global_Predictictions.csv')
        CORELABELS = dataAll.iloc[:, 0]
        dataAllY = dataAll.iloc[:, [4]] # This used to contain the core row (row 0)
        print(dataAllY)
        dataAllX = dataAll.iloc[:, [3, 6,7,10,11,14,15,17,18,19,20,24,25,26,27,28,29,30,34,35,37,38,43,44,45]]
...
layer_sizes = [16, 32, 64] # Possible layer sizes are in powers of two, including a size of 25 since there are about 25 inputs
activations = ['Li', 'Lo', 'T'] # Possible activation functions (0 meaning no activation)
batch_sizes  = [8, 16, 64] # Possible batch sizes are powers of two
                    ...
                    for act1 in activations:
                        ...
                        for layer2 in layer_sizes:
                            for act2 in activations:
                                ...
                                for layer3 in layer_sizes:
                                    for act3 in activations:
...
results.to_csv('ParameterTest1.csv')
